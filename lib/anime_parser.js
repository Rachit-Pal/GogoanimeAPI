...
export const scrapeDownloadLinks = async({ episodeId }) => {
    ...
    if (cachedDownloadLinks[episodeId]) {
        console.log(`Returning cached links for ${episodeId}: `)
        console.log(cachedDownloadLinks[episodeId])
    ...
    const browser = await puppeteer.launch({
        args: ['--enable-features=NetworkService', '--no-sandbox', '--disable-setuid-sandbox', '--disable-dev-shm-usage', '--disable-web-security', '--disable-features=IsolateOrigins,site-per-process'],
        dumpio: false,
    ...
    try {
        ...
        await d_page.setUserAgent(randomUseragent.getRandom() || USER_AGENT)
        await d_page.setJavaScriptEnabled(true);
        await d_page.reload({
            waitUntil: 'networkidle0',
        })
        d_page.setDefaultNavigationTimeout(0);
        await d_page.goto(d_link, { aitUntil: 'networkidle2' })

        console.log(await d_page.content())
